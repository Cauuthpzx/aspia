...
// for the Routes system-info widget: 6000..6099.
constexpr int kIdList = 6000;
...
SysInfoRoutes::SysInfoRoutes(HINSTANCE instance)
    : instance_(instance)
{
}

SysInfoRoutes::~SysInfoRoutes() = default;

bool SysInfoRoutes::create(HWND parent)
{
    ...
    wc.lpfnWndProc = &SysInfoRoutes::windowProc;
...
void SysInfoRoutes::move(int x, int y, int width, int height)
{
    if (!hwnd_)
        return;
...
// static
LRESULT CALLBACK SysInfoRoutes::windowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    SysInfoRoutes* self = nullptr;

    if (msg == WM_NCCREATE)
    {
        auto* cs = reinterpret_cast<CREATESTRUCTW*>(lp);
        self = static_cast<SysInfoRoutes*>(cs->lpCreateParams);
        self->hwnd_ = hwnd;
        SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(self));
    }
    else
    {
        self = reinterpret_cast<SysInfoRoutes*>(
...
LRESULT SysInfoRoutes::handleMessage(UINT msg, WPARAM wp, LPARAM lp)
{
    switch (msg)
    {
        case WM_CREATE:
...
void SysInfoRoutes::onSize(int width, int height)
{
    if (list_)
    {
...
void SysInfoRoutes::setRoutes(const std::vector<Route>& routes)
{
    if (!list_)
        return;
    ...
    for (size_t i = 0; i < routes.size(); ++i)
    {
        ...
        item.iSubItem = 0;
        item.pszText = const_cast<wchar_t*>(r.destination.c_str());

        const int row = ListView_InsertItem(list_, &item);
        if (row < 0)
            continue;

        setSubItem(list_, row, 1, r.netmask);
        setSubItem(list_, row, 2, r.gateway);
        setSubItem(list_, row, 3, r.metric);
    }
